...
public class Shader {
    ...
    public Shader(String filepathIn){
        this.filepath = filepathIn;
        try{
            String source = new String(Files.readAllBytes(Paths.get(filepath)));
            String [] splitString = source.split("(#type)( )+([a-zA-Z]+)");

            //Finds the first pattern after the #type 'pattern'
            int index = source.indexOf("#type") +6;
            int endOfLine = source.indexOf("\r\n", index);
            String firstPattern = source.substring(index, endOfLine).trim();

            //Finds the second pattern after the #type 'pattern'
    ...
    public void compile(){
        ...
        //Pastes the shader source to the GPU
        glShaderSource(vertexID,vertexSource);
        glCompileShader(vertexID);

        //Checks for errors in compilation
        ...
        //Links shaders and checks for errors.
